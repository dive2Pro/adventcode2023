...
export function Day5() {
  ...
  const actionString = inputs.substring(actionIndex);
  let result = reg.exec(stackString);
  while (result) {
    const index = (parseInt(result.index / 4) + 1) % 9 || 9;
    console.log(index, " ---");
    ary[index] = ary[index] || [];
    ary[index].push(result[1]);
    result = reg.exec(stackString);
  }
  const actions = [];
  const actionReg = /move (\d+) from (\d+) to (\d+)/gi;
